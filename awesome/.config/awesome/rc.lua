...
local editor = "neovim" or os.getenv("EDITOR") or "nano"
...
local mytextclock = wibox.widget.textclock('<span foreground="' .. beautiful.bg_normal .. '"> %a %b %d, %H:%M</span>')

-- local mytextclock = wibox.widget.textclock("%a %b %d %H:%M")
local month_calender = awful.widget.calendar_popup.month({
    long_weekdays = true,
    margin = beautiful.gap,
})

mytextclock:connect_signal("mouse::enter", function()
    month_calender:call_calendar(0, "tr", awful.screen.focused())
    month_calender.visible = true
end)

mytextclock:connect_signal("mouse::leave", function()
    month_calender.visible = false
end)

local spotifyWidget, spotifyTimer = awful.widget.watch(
    ...
    function(widget, stdout)
        ...
        widget:set_text(
            lines[songLine]:sub(44, string.len(lines[songLine]) - 1)
            .. "  -  "
            .. lines[artistLine]:sub(27, string.len(lines[artistLine]) - 1)
            .. "  "
        )
    end
)

local popupTasklist = awful.popup({
    widget = awful.widget.tasklist({
        screen = screen[1],
        --screen = awful.screen.focus,
...
awful.keygrabber({
    ...
    stop_key = "Mod1",
    stop_event = "release",
    --keyreleased_callback = function() popupTasklist.visible = false end,
    start_callback = awful.client.focus.history.disable_tracking,
    stop_callback = function()
        awful.client.focus.history.enable_tracking()
        popupTasklist.visible = false
    end,
...
globalkeys = gears.table.join(
    ...
    --	awful.key({ modkey }, "Tab", function()
    --		popupTasklist.screen = awful.screen.focused()
    --		popupTasklist.visible = not popupTasklist.visible
    --	end, { description = "open a terminal", group = "launcher" }),

    awful.key({ modkey }, "Escape", function()
        -- If you want to always position the menu on the same place set coordinates
        awful.menu.menu_keys.down = { "Down", "Alt_L" }
        awful.menu.clients({ theme = { width = 400, height = 24 } }, { keygrabber = true, coords = { x = 525, y = 330 } })
    end),
    -- Media keys
    ...
    awful.key({ any }, "XF86AudioStop", function()
        awful.spawn(
...
awful.rules.rules = {
    ...
    { rule = { name = "Spectacle" },                properties = { titlebars_enabled = true } },
